package defpackage;

import com.google.android.gms.internal.ads.zzfsf;
import com.google.android.gms.internal.ads.zzfxv;
import com.google.android.gms.internal.ads.zzfyx;

/* JADX INFO: loaded from: classes3.dex */
public final class rp6 extends tp6 {
    public rp6(zzfyx zzfyxVar, Class cls, zzfxv zzfxvVar) {
        super(zzfyxVar, cls, zzfxvVar);
    }

    @Override // defpackage.tp6
    public final /* bridge */ /* synthetic */ Object n(Object obj, Throwable th) {
        zzfxv zzfxvVar = (zzfxv) obj;
        zzfyx zzfyxVarZza = zzfxvVar.zza(th);
        zzfsf.zzd(zzfyxVarZza, "AsyncFunction.apply returned null instead of a Future. Did you mean to return immediateFuture(null)? %s", zzfxvVar);
        return zzfyxVarZza;
    }

    @Override // defpackage.tp6
    public final /* synthetic */ void o(Object obj) {
        j((zzfyx) obj);
    }
}
