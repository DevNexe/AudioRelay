...
public class k47 extends AbstractMap {
    public static final /* synthetic */ int C = 0;
    public volatile j47 A;
    public final int w;
    public boolean z;
    ...
    public final Object put(Comparable comparable, Object obj) {
        ...
        int iC = c(comparable);
        if (iC >= 0) {
            return ((h47) this.x.get(iC)).setValue(obj);
        }
        g();
        ...
        if (this.x.size() == i) {
            h47 h47Var = (h47) this.x.remove(i - 1);
            f().put(h47Var.w, h47Var.x);
        }
        this.x.add(i2, new h47(this, comparable, obj));
        return null;
    }
    ...
    public final int c(Comparable comparable) {
        ...
        int i = 0;
        if (size >= 0) {
            int iCompareTo = comparable.compareTo(((h47) this.x.get(size)).w);
            if (iCompareTo > 0) {
                return -(size + 2);
            }
            if (iCompareTo == 0) {
        ...
        while (i <= size) {
            int i2 = (i + size) / 2;
            int iCompareTo2 = comparable.compareTo(((h47) this.x.get(i2)).w);
            if (iCompareTo2 < 0) {
                size = i2 - 1;
            } else {
                if (iCompareTo2 <= 0) {
    ...
    public final Object e(int i) {
        g();
        Object obj = ((h47) this.x.remove(i)).x;
        if (!this.y.isEmpty()) {
            Iterator it = f().entrySet().iterator();
            List list = this.x;
            Map.Entry entry = (Map.Entry) it.next();
            list.add(new h47(this, (Comparable) entry.getKey(), entry.getValue()));
            it.remove();
        }
        return obj;
    }
    ...
    public final boolean equals(Object obj) {
        ...
        if (size2 != k47Var.x.size()) {
            return entrySet().equals(k47Var.entrySet());
        }
        for (int i = 0; i < size2; i++) {
            if (!((Map.Entry) this.x.get(i)).equals((Map.Entry) k47Var.x.get(i))) {
                return false;
            }
        }
        if (size2 != size) {
            return this.y.equals(k47Var.y);
        }
        return true;
    }
    ...
    public final Object get(Object obj) {
        Comparable comparable = (Comparable) obj;
        int iC = c(comparable);
        return iC >= 0 ? ((h47) this.x.get(iC)).x : this.y.get(comparable);
    }
    ...
    public final int hashCode() {
        ...
        for (int i = 0; i < size; i++) {
            iHashCode += ((h47) this.x.get(i)).hashCode();
        }
