...
public class RadarHighlighter extends PieRadarHighlighter<RadarChart> {
    ...
    public List<Highlight> getHighlightsAtIndex(int i) {
        ...
        for (int i2 = 0; i2 < ((RadarData) ((RadarChart) this.mChart).getData()).getDataSetCount(); i2++) {
            ...
            float f = i;
            Utils.getPosition(((RadarChart) this.mChart).getCenterOffsets(), (entryForIndex.getY() - ((RadarChart) this.mChart).getYChartMin()) * factor * phaseY, ((RadarChart) this.mChart).getRotationAngle() + (sliceAngle * f * phaseX), mPPointF);
