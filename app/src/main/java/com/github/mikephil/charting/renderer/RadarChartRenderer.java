...
public class RadarChartRenderer extends LineRadarRenderer {
    ...
    public void drawDataSet(Canvas canvas, IRadarDataSet iRadarDataSet, int i) {
        ...
        for (int i2 = 0; i2 < iRadarDataSet.getEntryCount(); i2++) {
            ...
            Utils.getPosition(centerOffsets, (((RadarEntry) iRadarDataSet.getEntryForIndex(i2)).getY() - this.mChart.getYChartMin()) * factor * phaseY, this.mChart.getRotationAngle() + (i2 * sliceAngle * phaseX), mPPointF);
        ...
        while (i2 < length) {
            ...
            if (dataSetByIndex != null && dataSetByIndex.isHighlightEnabled()) {
                ...
                if (isInBoundsX(entry, dataSetByIndex)) {
                    Utils.getPosition(centerOffsets, this.mAnimator.getPhaseY() * (entry.getY() - this.mChart.getYChartMin()) * factor, this.mChart.getRotationAngle() + (this.mAnimator.getPhaseX() * highlight.getX() * sliceAngle), mPPointF);
        ...
        while (i2 < ((RadarData) this.mChart.getData()).getDataSetCount()) {
            ...
            if (shouldDrawValues(dataSetByIndex)) {
                ...
                while (i3 < dataSetByIndex.getEntryCount()) {
                    ...
                    Utils.getPosition(centerOffsets, (radarEntry.getY() - this.mChart.getYChartMin()) * factor * phaseY, this.mChart.getRotationAngle() + f2, mPPointF);
                    ...
                    if (radarEntry.getIcon() != null && dataSetByIndex.isDrawIconsEnabled()) {
                        ...
                        Utils.getPosition(centerOffsets, (radarEntry.getY() * factor * phaseY) + mPPointF4.y, this.mChart.getRotationAngle() + f2, mPPointF2);
                        float f3 = mPPointF2.y + mPPointF4.x;
                        mPPointF2.y = f3;
                        Utils.drawImage(canvas, icon, (int) mPPointF2.x, (int) f3, icon.getIntrinsicWidth(), icon.getIntrinsicHeight());
                    }
                    i3++;
                    mPPointF3 = mPPointF4;
