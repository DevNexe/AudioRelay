...
public class ViewPortHandler {
    ...
    public void limitTransAndScale(Matrix matrix, RectF rectF) {
        ...
        this.mTransX = Math.min(Math.max(f, ((this.mScaleX - 1.0f) * (-fWidth)) - this.mTransOffsetX), this.mTransOffsetX);
        float fMax = Math.max(Math.min(f3, ((this.mScaleY - 1.0f) * fHeight) + this.mTransOffsetY), -this.mTransOffsetY);
        this.mTransY = fMax;
