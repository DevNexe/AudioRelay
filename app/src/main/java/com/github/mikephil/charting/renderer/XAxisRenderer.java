...
public class XAxisRenderer extends AxisRenderer {
            ...
            if (this.mViewPortHandler.isInBoundsX(f2)) {
                ...
                if (this.mXAxis.isAvoidFirstLastClippingEnabled()) {
                    ...
                    if (i4 == i5 - 1 && i5 > 1) {
                        ...
                        if (fCalcTextWidth2 > this.mViewPortHandler.offsetRight() * 2.0f && f2 + fCalcTextWidth2 > this.mViewPortHandler.getChartWidth()) {
                            f2 -= fCalcTextWidth2 / 2.0f;
                        }
                    } else if (i3 == 0) {
                        fCalcTextWidth = (Utils.calcTextWidth(this.mAxisLabelPaint, axisLabel) / 2.0f) + f2;
                    }
                    fCalcTextWidth = f2;
                } else {
                    fCalcTextWidth = f2;
    ...
    public void renderLimitLineLabel(Canvas canvas, LimitLine limitLine, float[] fArr, float f) {
        ...
        if (labelPosition == LimitLine.LimitLabelPosition.RIGHT_TOP) {
            ...
            canvas.drawText(label, fArr[0] + xOffset, this.mViewPortHandler.contentTop() + f + fCalcTextHeight, this.mLimitLinePaint);
        } else if (labelPosition == LimitLine.LimitLabelPosition.RIGHT_BOTTOM) {
            this.mLimitLinePaint.setTextAlign(Paint.Align.LEFT);
            canvas.drawText(label, fArr[0] + xOffset, this.mViewPortHandler.contentBottom() - f, this.mLimitLinePaint);
        } else if (labelPosition != LimitLine.LimitLabelPosition.LEFT_TOP) {
            this.mLimitLinePaint.setTextAlign(Paint.Align.RIGHT);
            canvas.drawText(label, fArr[0] - xOffset, this.mViewPortHandler.contentBottom() - f, this.mLimitLinePaint);
        } else {
            this.mLimitLinePaint.setTextAlign(Paint.Align.RIGHT);
            canvas.drawText(label, fArr[0] - xOffset, this.mViewPortHandler.contentTop() + f + Utils.calcTextHeight(this.mLimitLinePaint, label), this.mLimitLinePaint);
        }
    }
