    ...
    public void renderAxisLabels(Canvas canvas) {
        if (this.mXAxis.isEnabled() && this.mXAxis.isDrawLabelsEnabled()) {
            ...
            for (int i = 0; i < ((RadarData) this.mChart.getData()).getMaxEntryCountSet().getEntryCount(); i++) {
                ...
                Utils.getPosition(centerOffsets, (this.mXAxis.mLabelRotatedWidth / 2.0f) + (this.mChart.getYRange() * factor), (this.mChart.getRotationAngle() + (f * sliceAngle)) % 360.0f, mPPointF2);
