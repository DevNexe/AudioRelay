    ...
    public static void initialize(Context context, String str, boolean z, IUnityAdsInitializationListener iUnityAdsInitializationListener) {
        ...
        if (SdkProperties.getCurrentInitializationState() != SdkProperties.InitializationState.NOT_INITIALIZED) {
            ...
            if (gameId != null && !gameId.equals(str)) {
                string = "" + createExpectedParametersString("Game ID", ClientProperties.getGameId(), str);
            }
            boolean zIsTestMode = SdkProperties.isTestMode();
            if (zIsTestMode != z) {
                StringBuilder sbG = Md5A.g(string);
                sbG.append(createExpectedParametersString("Test Mode", Boolean.valueOf(zIsTestMode), Boolean.valueOf(z)));
                string = sbG.toString();
            }
            if (!TextUtils.isEmpty(string)) {
                String str2 = "Unity Ads SDK failed to initialize due to already being initialized with different parameters" + string;
                DeviceLog.warning(str2);
                if (iUnityAdsInitializationListener != null) {
                    iUnityAdsInitializationListener.onInitializationFailed(UnityAds.UnityAdsInitializationError.INVALID_ARGUMENT, str2);
                    return;
                }
                return;
            }
