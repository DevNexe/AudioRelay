...
public class Intent {
    ...
    public static void canOpenIntents(JSONArray jSONArray, WebViewCallback webViewCallback) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            try {
                jSONObject.put(jSONObjectOptJSONObject.optString(FacebookAdapter.KEY_ID), checkIntentResolvable(intentFromMetadata(jSONObjectOptJSONObject)));
            } catch (IntentException e) {
    ...
    private static boolean setExtras(android.content.Intent intent, JSONArray jSONArray) {
        ...
        for (int i = 0; i < jSONArray.length(); i++) {
            try {
                ...
                if (!setExtra(intent, jSONObject.getString(SubscriberAttributeKt.JSON_NAME_KEY), jSONObject.get("value"))) {
                    return false;
                }
            } catch (Exception e) {
