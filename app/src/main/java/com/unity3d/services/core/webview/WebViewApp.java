...
public class WebViewApp implements IWebViewBridgeInvoker {
    ...
    private static ErrorState createWithRemoteUrl(final Configuration configuration) {
        ...
        Utilities.runOnUiThread(new Runnable() { // from class: com.unity3d.services.core.webview.WebViewApp.2
            ...
            public void run() {
                try {
                    WebViewApp webViewApp = new WebViewApp(configuration, true);
        ...
        return !zBlock ? ErrorState.CreateWebviewTimeout : getCurrentApp().getErrorStateFromWebAppCode();
    }
    ...
    private WebViewApp(Configuration configuration, boolean z) {
        this._webAppLoaded = false;
        setConfiguration(configuration);
        WebViewBridge.setClassTable(getConfiguration().getWebAppApiClassList());
        WebView webViewWithCache = z ? new WebViewWithCache(ClientProperties.getApplicationContext()) : new WebView(ClientProperties.getApplicationContext());
        ...
        Utilities.runOnUiThread(new Runnable() { // from class: com.unity3d.services.core.webview.WebViewApp.1
            ...
            public void run() {
                try {
                    ...
                    WebViewApp webViewApp = new WebViewApp(configuration2, configuration2.getExperiments().isWebAssetAdCaching());
                    ...
                    if (urlWithQueryString == null) {
                        ...
                        webViewApp.getWebView().loadDataWithBaseURL(urlWithQueryString, configuration.getWebViewData(), "text/html", "UTF-8", null);
                    }
                    WebViewApp.setCurrentApp(webViewApp);
                } catch (Exception unused) {
                    DeviceLog.error("Couldn't construct WebViewApp");
