...
public final class zzefk {
    ...
    public static int zza(SQLiteDatabase sQLiteDatabase, int i) {
        ...
        Cursor cursorA = a(sQLiteDatabase, i);
        if (cursorA.getCount() > 0) {
            cursorA.moveToNext();
            i2 = cursorA.getInt(cursorA.getColumnIndexOrThrow("value"));
        }
        cursorA.close();
        return i2;
    }
    ...
    public static long zzb(SQLiteDatabase sQLiteDatabase, int i) {
        ...
        if (cursorA.getCount() > 0) {
            cursorA.moveToNext();
            j = cursorA.getLong(cursorA.getColumnIndexOrThrow("value"));
        } else {
            j = 0;
        }
        cursorA.close();
    ...
    public static ArrayList zzc(SQLiteDatabase sQLiteDatabase) {
        ...
        while (cursorQuery.moveToNext()) {
            try {
                arrayList.add(zzbgu.zzi(cursorQuery.getBlob(cursorQuery.getColumnIndexOrThrow("serialized_proto_data"))));
            } catch (zzgoz e) {
                zzcgn.zzg("Unable to deserialize proto from offline signals database:");
                zzcgn.zzg(e.getMessage());
            }
        }
        cursorQuery.close();
        return arrayList;
    ...
    public static void zze(SQLiteDatabase sQLiteDatabase) {
        b(sQLiteDatabase, "failed_requests");
        b(sQLiteDatabase, "total_requests");
        b(sQLiteDatabase, "completed_requests");
        ContentValues contentValues = new ContentValues();
    ...
    public static void zzf(SQLiteDatabase sQLiteDatabase) {
        ...
        c(sQLiteDatabase, "failed_requests");
        c(sQLiteDatabase, "total_requests");
        c(sQLiteDatabase, "completed_requests");
    }
