...
public final class SF {
    ...
    public static long A01(List<SG> list) {
        HashMap map = new HashMap();
        for (int i = 1; i < list.size(); i++) {
            long jA03 = list.get(i).A03();
            long jA01 = list.get(i).A01();
        ...
        for (int i3 = 1; i3 < i; i3++) {
            list.get(i3).A08(list.get(i3).A03() - jLongValue);
            list.get(i3).A06(list.get(i3).A01() - jLongValue);
        }
        return jLongValue;
    }
    ...
    public static String A04(@Nullable List<SG> list) {
        if (list == null || list.isEmpty()) {
            return null;
        }
        HashMap map = new HashMap();
        map.put(A02(73, 2, 35), list.get(0).A04());
        map.put(A02(92, 7, 20), 1);
        boolean z = false;
        if (list.size() > 1) {
            A09(list);
            map.put(A02(64, 9, 58), Long.valueOf(A01(list)));
            ...
            map.put(A02(83, 9, 29), Boolean.valueOf(z));
        }
        String strA06 = A06(map);
        if (strA06.length() > 900000) {
            String encodedFrameData = A02(99, 29, 126);
