...
public final class C0695Iz {
    ...
    public static long A0F(long multiplicationFactor, long j, long j2) {
        if (j2 >= j && j2 % j == 0) {
            long divisionFactor = multiplicationFactor / (j2 / j);
        ...
        return (long) (multiplicationFactor * (d / j2));
    }
    ...
    public static long A0I(long maxPositionUs, C0506Ax c0506Ax, long j, long j2) {
        if (C0506Ax.A04.equals(c0506Ax)) {
            return maxPositionUs;
        }
        long jA0H = A0H(maxPositionUs, c0506Ax.A01, Long.MIN_VALUE);
        long jA0G = A0G(maxPositionUs, c0506Ax.A00, Long.MAX_VALUE);
        boolean z = true;
        boolean z2 = jA0H <= j && j <= jA0G;
        if (jA0H > j2 || j2 > jA0G) {
            z = false;
        }
        if (z2 && z) {
            if (Math.abs(j - maxPositionUs) <= Math.abs(j2 - maxPositionUs)) {
                return j;
            }
            return j2;
        }
    ...
    public static ExecutorService A0T(String str) {
        return Executors.newSingleThreadExecutor(new ThreadFactoryC0694Iy(str));
    }
    ...
    public static void A0W(InterfaceC0650He interfaceC0650He) {
    ...
    public static byte[] A0h(String str) {
        ...
        for (int i = 0; i < i; i++) {
            int i2 = i * 2;
            int stringOffset = Character.digit(str.charAt(i2), 16);
            int i3 = stringOffset << 4;
            int stringOffset2 = i2 + 1;
            bArr[i] = (byte) (i3 + Character.digit(str.charAt(stringOffset2), 16));
        }
        return bArr;
    }
