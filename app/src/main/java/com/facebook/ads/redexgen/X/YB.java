    ...
    public final boolean A0A(C7A<HashMap<String, Integer>> c7a) {
        if (A07() != null) {
            ...
            if (mapA07 != null) {
                ...
                HashMap<String, Integer> mapA08 = c7a.A07();
                HashMap<String, Integer> mapA09 = A07();
                Iterator<String> it = mapA08.keySet().iterator();
                Iterator<String> prevSignalValueKeys = mapA09.keySet().iterator();
                while (it.hasNext() && prevSignalValueKeys.hasNext()) {
                    String newSignalValueKey = it.next();
                    String next = prevSignalValueKeys.next();
                    if (!newSignalValueKey.equals(next) || !mapA08.containsKey(newSignalValueKey) || !mapA09.containsKey(next) || !mapA08.get(newSignalValueKey).equals(mapA09.get(next))) {
                        return false;
                    }
                }
                return true;
