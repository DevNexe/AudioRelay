...
public class C04769t {
    ...
    public C04769t(RandomAccessFile randomAccessFile, long[] jArr, long j) throws IOException {
    ...
    public static C04769t A03(File file) throws IOException {
        RandomAccessFile randomAccessFile = new RandomAccessFile(file, A05(416, 3, 41));
        try {
            randomAccessFile.writeLong(-360011992771067903L);
            randomAccessFile.writeLong(C04779u.A03);
            randomAccessFile.write(new byte[C04779u.A03 * 8]);
            randomAccessFile.getFD().sync();
            return new C04769t(randomAccessFile, new long[C04779u.A03], randomAccessFile.length());
    ...
    public static C04769t A04(File file) throws IOException {
        try {
            RandomAccessFile randomAccessFile = new RandomAccessFile(file, A05(416, 3, 41));
            if (file.length() >= A00()) {
                ...
                if (j2 < 0 || j2 > 100000) {
                    throw new IOException(String.format(Locale.US, A05(77, 40, 47), Long.valueOf(j2)));
                }
                long length = randomAccessFile.length();
                long recordCount = A02((int) j2);
                if (length >= recordCount) {
                    long[] jArr = new long[(int) j2];
                    for (int i = 0; i < i; i++) {
                        jArr[i] = randomAccessFile.readLong();
                    }
                    return new C04769t(randomAccessFile, jArr, randomAccessFile.length());
                }
                throw new IOException(String.format(Locale.US, A05(117, 63, 43), Long.valueOf(j2), Long.valueOf(file.length())));
            }
