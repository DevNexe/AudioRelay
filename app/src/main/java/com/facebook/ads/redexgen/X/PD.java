...
public final class PD {
    ...
    @Nullable
    @RequiresApi(api = 21)
    public static WebResourceResponse A00(C1075Xy c1075Xy, WebResourceRequest webResourceRequest, Uri uri, String str, HashMap<String, String> map) throws IOException {
        String strA02;
        String strA03 = A02(85, 5, 49);
        C0883Qk exoPlayerCacheManager = C0883Qk.A05(c1075Xy.A00());
        InterfaceC0649Hd dataSourceFactory = exoPlayerCacheManager.A0F(c1075Xy);
        C7U.A0H(c1075Xy, uri.toString());
        try {
            PE pe = new PE(c1075Xy.A00(), uri, dataSourceFactory);
            int iAvailable = pe.available();
            if (iAvailable <= 0) {
                A05(c1075Xy, 1, new Pair[]{new Pair(A02(61, 9, 77), String.valueOf(iAvailable))});
                return null;
            }
            String strA04 = A03(webResourceRequest.getRequestHeaders());
            if (strA04 != null) {
                try {
                    PC pcA01 = A01(strA04);
                    if (!pcA01.A03) {
                    ...
                    c1075Xy.A0D().A53();
                    return new WebResourceResponse(str, null, 206, A02(46, 15, 36), map, pe);
                } catch (NumberFormatException e) {
                    A05(c1075Xy, 3, new Pair[]{new Pair(strA03, e.toString())});
                    return null;
                }
            }
            c1075Xy.A0D().A53();
            A06(map, iAvailable);
            return new WebResourceResponse(str, null, RCHTTPStatusCodes.SUCCESS, A02(44, 2, 15), map, pe);
        } catch (IOException e2) {
            A05(c1075Xy, 2, new Pair[]{new Pair(strA03, e2.toString())});
            return null;
        }
    }
    ...
    public static void A05(C1075Xy c1075Xy, int i, Pair<String, String>[] pairArr) {
        ...
        c1075Xy.A0D().A52(jSONObject.toString());
    }
